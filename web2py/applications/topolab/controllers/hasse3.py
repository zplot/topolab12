...
def topo1():
    form = SQLFORM.factory(
        Field('puntos', requires=IS_NOT_EMPTY()),
        Field('base_minima', requires=IS_NOT_EMPTY()))
    if form.process().accepted:
        response.flash = 'form accepted'
        session.puntos = form.vars.puntos
        session.base_minima = form.vars.base_minima
        redirect(URL('topo4'))
    elif form.errors:
...
def prueba1():
    ...
    return my_hasse1(session.puntos, session.base_minima)
