...
class Topo(object):
    ...
    '''
    # logger.info('Entro en la clase Topo')
    log.info('Entro en la clase Topo *****************************************************')

    def __init__(self, puntos, abiertos):
        # Nos salimos si no es espacio o si no es T0
        if not es_espacio_topologico(puntos, abiertos):
            log.info('Sorry, it is not a topological space')
            log.info('Cortamos y salimos')
            exit()
        if not es_t0(puntos, abiertos):
            log.info('Sorry, this space is not T0')
            log.info('Cortamos y salimos')
            exit()
        ...
        # logger.info('Salgo de la clase Topo')
        return
