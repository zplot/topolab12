...
def form_processing_es_una_base(form):
    ...
    if not tiene_pinta_de_puntos(puntos1):
        form.errors.points = 'Does not seem to be a list of points'
        return
    if not tiene_pinta_de_base(base1):
        form.errors.basis = 'Does not seem to be a list of open sets'
        return
    ...
    try:
        ...
        form.errors.points = 'Your input is not right. Please try again'
        form.errors.basis = 'Your input is not right. Please try again'
        response.flash = 'form has errors'
